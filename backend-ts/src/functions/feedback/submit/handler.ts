...
export const handler = requireAuth(
  async (event: AuthenticatedEvent, _context: Context): Promise<APIGatewayProxyResult> => {
    try {
      ...
      // Create new like with deterministic ID to prevent duplicates
      // ID is based on user, location, and type so the same user can't create multiple likes
      const feedbackId = `${user.id}_${locationId}_like`;
